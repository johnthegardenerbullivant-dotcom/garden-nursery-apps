...
import {
    ...
    getChildBatches, updateNurseryBatch,
...
export async function renderBatchDetail(container, headerActionEl, backBtn, id) {
    ...
    // FIX 2: For linked-plant batches, botanical name is the primary display name
    const displayName      = (batch.plantId && batch.botanicalName)
        ? batch.botanicalName
        : (batch.plantName || 'Unnamed');
    const displaySecondary = (batch.plantId && batch.botanicalName)
        ? (batch.commonName || '')
        : (batch.commonName && batch.commonName !== batch.plantName ? batch.commonName : '');
    const showBotanicalLine = !batch.plantId && !!batch.botanicalName;
    ...
    container.innerHTML = `
        <div class="view-content">
            ...
            <div class="detail-header-card">
                ...
                ${batch.hybrid && !batch.botanicalName ? `<p class="detail-botanical">× Hybrid</p>` : ''}
            ...
            <div class="detail-grid">
                ...
                <div class="detail-field">
                    ...
                    <span class="detail-value detail-qty-value">${batch.currentQty ?? batch.startQty ?? '—'} of ${batch.startQty ?? '—'} started</span>
                </div>
                ${batch.medium ? `
                <div class="detail-field">
            ...
            sec.innerHTML = children.map(c => `
                <div class="batch-row card-item" data-id="${escHtml(c.id)}" style="cursor:pointer;">
                    <div class="batch-row-main">
                        <span class="batch-row-name">${escHtml(c.plantName || 'Unnamed')}</span>
                    ...
                    <div class="batch-row-meta">
                        ...
                        <span class="batch-row-qty">${c.currentQty ?? c.startQty ?? '?'} plants</span>
                    </div>
                </div>
            `).join('');
    ...
    container.querySelector('#record-outcome-btn')?.addEventListener('click', () => {
        showOutcomeForm(batch, async () => {
            batch = await getNurseryBatch(id);
            await loadAndRenderOutcomes(container, batch);
            await loadAndRenderLogs(container, batch);
            const qtyEl = container.querySelector('.detail-qty-value');
            if (qtyEl) qtyEl.textContent = `${batch.currentQty ?? batch.startQty ?? '—'} of ${batch.startQty ?? '—'} started`;
            const stageEl = container.querySelector('.detail-stage-badge');
            if (stageEl) {
                stageEl.textContent = STAGE_LABELS[batch.stage] || batch.stage || '—';
                stageEl.className = `stage-badge stage-${batch.stage || 'propagating'} detail-stage-badge`;
            }
        });
    });
    ...
    container.querySelector('#add-log-btn')?.addEventListener('click', () => {
        showLogForm(batch, async () => {
            batch = await getNurseryBatch(id);
            await loadAndRenderLogs(container, batch);
            loadAndRenderBatchPhotos(container, batch);  // FIX 1: refresh merged photos
            const qtyEl = container.querySelector('.detail-qty-value');
            if (qtyEl) qtyEl.textContent = `${batch.currentQty ?? batch.startQty ?? '—'} of ${batch.startQty ?? '—'} started`;
            const stageEl = container.querySelector('.detail-stage-badge');
            if (stageEl) {
    ...
    loadAndRenderLogs(container, batch);
...
async function loadAndRenderLogs(container, batch) {
    ...
    section.querySelectorAll('.log-edit-btn').forEach(btn => {
        btn.addEventListener('click', e => {
            ...
            if (log) showEditLogForm(log, batch, async () => {
                batch = await getNurseryBatch(batch.id);
                logs  = await getLogsForBatch(batch.id);
                await loadAndRenderLogs(container, batch);
                loadAndRenderBatchPhotos(container, batch);  // FIX 1: refresh merged photos
                const qtyEl = container.querySelector('.detail-qty-value');
