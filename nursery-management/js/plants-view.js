...
import { getNurseryBatches, escHtml, fmtDate, METHOD_LABELS, STAGE_LABELS } from './db.js';
    ...
    return `
        <div class="plant-card card-item" data-key="${escHtml(p.plantId || p.plantName)}">
            <div class="plant-card-main">
                <div>
                    <div class="plant-card-name">${escHtml(p.plantName)}</div>
                    ${p.botanicalName ? `<div class="plant-card-botanical">${escHtml(p.botanicalName)}</div>` : ''}
                </div>
...
export async function renderPlantPropHistory(container, headerActionEl, backBtn, key) {
    ...
    const plantName    = batches[0].plantName || 'Unknown';
    const botanicalName = batches[0].botanicalName || '';
    ...
    container.innerHTML = `
        <div class="view-content">
            <div class="detail-header-card">
                ...
                ${botanicalName ? `<p class="detail-botanical">${escHtml(botanicalName)}</p>` : ''}
