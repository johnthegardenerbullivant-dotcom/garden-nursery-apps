...
import {
    ...
    plantOutToGarden, updateNurseryBatch, formatBatchQty,
    ...
    section.querySelectorAll('.outcome-delete-btn').forEach(btn => {
        btn.addEventListener('click', async e => {
            ...
            try {
                ...
                if (qtyEl) qtyEl.textContent = formatBatchQty(batch);
...
function outcomeCard(o) {
    ...
    const editBtn = isAtLeast('editor') ? `
        <button class="btn-icon outcome-edit-btn" title="Edit outcome">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
        </button>` : '';
    const deleteBtn = isAtLeast('admin') ? `
        <button class="btn-icon danger outcome-delete-btn" title="Delete outcome">
        ...
        <form id="outcome-form" novalidate>
            ...
            <div class="form-row-two">
                ...
                <div class="form-group">
                    ...
                    ${datePicker('outcome-date', 'date', initDate)}
                </div>
            </div>
...
async function showOutcomeForm(batch, onSaved) {
    ...
    document.getElementById('outcome-form')?.addEventListener('submit', async e => {
        ...
        const date  = document.getElementById('outcome-date')?.value.trim() || '';
