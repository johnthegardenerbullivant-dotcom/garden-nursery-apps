...
import {
    ...
    METHOD_LABELS, STAGE_LABELS, STAGE_ORDER, escHtml, fmtDate, formatBatchQty, formatBotanicalName
} from './db.js';
...
export async function renderBatchesList(container, headerActionEl, backBtn) {
    ...
    if (isAtLeast('editor')) {
        ...
        fab.title     = 'New batch';
        fab.innerHTML = '+';
        fab.addEventListener('click', () => showBatchForm(null, locations, onBatchSaved));
    ...
    container.innerHTML = `
        <div class="view-content view-content--list">
            ...
            <div class="stage-filter-pills" id="origin-pills" style="display:none;">
                <button class="filter-pill ${currentOrigin === 'all'      ? 'active' : ''}" data-origin="all">All</button>
                <button class="filter-pill ${currentOrigin === 'created'  ? 'active' : ''}" data-origin="created">🌱 Created</button>
                <button class="filter-pill ${currentOrigin === 'acquired' ? 'active' : ''}" data-origin="acquired">🪴 Acquired</button>
            </div>
    ...
    function render() {
        ...
        // Origin filter (completed tab only): acquired = method 'acquired-potted', created = anything propagated
        if (currentStatus === 'completed' && currentOrigin !== 'all') {
            const wantAcquired = currentOrigin === 'acquired';
            filtered = filtered.filter(b => (b.method === 'acquired-potted') === wantAcquired);
        }
        if (searchQuery) {
        ...
        const originPillsEl = container.querySelector('#origin-pills');
        if (originPillsEl) originPillsEl.style.display = (currentStatus === 'completed') ? 'flex' : 'none';
    ...
    container.querySelectorAll('#origin-pills .filter-pill').forEach(btn => {
        btn.addEventListener('click', () => {
            currentOrigin = btn.dataset.origin;
            container.querySelectorAll('#origin-pills .filter-pill').forEach(b =>
                b.classList.toggle('active', b.dataset.origin === currentOrigin));
            saveState();
            render();
        });
    });
...
function batchRow(b) {
    ...
    const isAcquired = b.method === 'acquired-potted';
    const originTag = `<span class="origin-tag origin-${isAcquired ? 'acquired' : 'created'}">${isAcquired ? 'Acquired' : 'Created'}</span>`;
    return `
        <div class="batch-row card-item" data-id="${escHtml(b.id)}">
            ...
            ${b.commonName && b.commonName !== (formatBotanicalName(b) || b.plantName) ? `<div class="batch-row-botanical">${escHtml(b.commonName)}</div>` : ''}
        </div>
    `;
}
