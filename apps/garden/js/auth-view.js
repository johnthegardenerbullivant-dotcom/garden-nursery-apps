...
/** Show the login overlay and bind interaction events. */
export function showLoginOverlay() {
    const overlay = document.getElementById('login-overlay');
    if (!overlay) return;
    // Reset to the default login card (in case access-denied was showing)
    overlay.querySelector('.login-card').innerHTML = buildLoginCardHTML();
    overlay.style.display = 'flex';
    bindLoginEvents(overlay);
}

/** Hide the login overlay (called once auth + role are confirmed). */
...
/**
 * Replace the login card with an "Access Pending" message.
 * Shown when a user has authenticated but has no role document yet.
 */
export function showAccessDenied(email) {
    ...
    overlay.querySelector('.login-card').innerHTML = `
        ...
        <p class="login-subtitle" style="text-align:center;margin-bottom:24px;line-height:1.6;">
            ...
            Please ask the garden admin to assign you a role in the Firebase Console.
        </p>
...
function buildLoginCardHTML() {
    return `
        <div class="login-logo">🌿</div>
        <h1 class="login-title">Garden Inventory</h1>
        <p class="login-subtitle">Sign in to access the garden</p>
...
function bindLoginEvents(overlay) {
    ...
    overlay.querySelector('#guest-signin-btn')?.addEventListener('click', async () => {
        ...
        try {
            await signInAsGuest();
            // onAuthStateChanged in main.js takes over — will auto-assign viewer role
        } catch (e) {
